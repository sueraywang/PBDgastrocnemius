...
class Simulator:
    ...
    def step(self):
        ...
        """
            ...
            movement = wp.vec3(0.0, -0.0005, 0.0)
        else:
            movement = wp.vec3(0.0, 0.0005, 0.0)
        """
        ...
        for _ in range(SUB_STEPS):
            ...
            self.iterative_jacobi_solve(device="cuda", kernel=solve_material_constraints, dim=self.numTets, numIterations=5,
            ...
            #"""
            self.iterative_jacobi_solve(device="cuda", kernel=solve_collision_constraints, dim=self.numVertices, numIterations=5, jacobiScale=0.2,
    ...
    def detect_collisions(self):
        ...
        wp.launch(
            ...
            inputs=[
                ...
                self.BVH.id,
                self.faceIds,
                self.faceOffsets,     # Added
                self.vertexOffsets,   # Added
                self.collisionThreshold,
                DT,
...
def detect_vertex_triangle_collisions(
    ...
    while wp.mesh_query_aabb_next(query, tri_index):
        ...
        dist = wp.length(closest_p - vertex)
        ...
        if dist < threshold:
            ...
            wp.atomic_min(triangle_collision_distances, tri_index, dist)
    ...
    vertex_collision_distances[vNr] = min_dis_to_tris
