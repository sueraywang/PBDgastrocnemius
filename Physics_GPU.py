...
class Simulator:
    ...
    def step(self):
        ...
        for _ in range(SUB_STEPS):
            ...
            self.iterative_jacobi_solve(device="cuda", kernel=solve_vol_constraints, dim=self.numTets, numIterations=5,
                                    inputs=[self.positions, self.invMasses, self.tetIds, self.invRestPoses,
                                            ...
                                            dt, self.corrections])
            self.iterative_jacobi_solve(device="cuda", kernel=solve_dev_constraints, dim=self.numTets, numIterations=5,
                                    inputs=[self.positions, self.invMasses, self.tetIds, self.invRestPoses,
                                            self.invRestVolumes, self.volCompliance, self.devCompliance, 
                                            dt, self.corrections])
...
@wp.kernel
def solve_vol_constraints(vertices: wp.array(dtype=wp.vec3),
    ...
    product = wp.transpose(invRestPoses[tetNr]) @ dF
    """
    ...
    if C != 0.0:
        ...
        if w > 0.0:
            alpha = volCompliance * invRestVolumes[tetNr] / dt / dt
            dlambda = -C / (w + alpha)
...
@wp.kernel
def solve_dev_constraints(vertices: wp.array(dtype=wp.vec3),
                        invMasses: wp.array(dtype=wp.float32),
                        tetIds: wp.array2d(dtype=wp.int32),
                        invRestPoses: wp.array(dtype=wp.mat33),
                        invRestVolumes: wp.array(dtype=wp.float32),
    ...
    if C != 0.0:
        ...
        if w > 0.0:
            alpha = devCompliance * invRestVolumes[tetNr] / dt / dt
            dlambda = -C / (w + alpha)
