...
class Simulator:
    ...
    def init_simulator(self):
        ...
        self.upper_bounds = wp.zeros(self.numFaces, dtype=wp.vec3, device=self.device)
        self.lower_bounds = wp.zeros(self.numFaces, dtype=wp.vec3, device=self.device)
        wp.launch(
            kernel=build_triangle_bounds,
            dim=self.numFaces,
            inputs=[self.positions, self.faceIds, self.lower_bounds, self.upper_bounds],
            device=self.device
        )
        # Create BVH
...
def build_triangle_bounds(
    ...
    # Get triangle vertices
    v0 = vertices[faces[fNr, 0]]
    v1 = vertices[faces[fNr, 1]]
    v2 = vertices[faces[fNr, 2]]
    
    # Compute bounds
    lower_bounds[fNr] = wp.min(wp.min(v0, v1), v2)
    upper_bounds[fNr] = wp.max(wp.max(v0, v1), v2)
...
def count_intersections(
    ...
    """Count ray intersections with the mesh surface"""
    query = wp.bvh_query_ray(bvh_id, origin, dir)
    face_idx = wp.int32(0)
    
    while wp.bvh_query_next(query, face_idx):
        # Get triangle vertices
        face = faces[face_idx]
        ...
        if t > 1e-6:  # Valid intersection
            wp.atomic_add(intersection_count, 0, 1)

@wp.kernel
def detect_and_compact_inside_vertices(
    positions: wp.array(dtype=wp.vec3),
    velocities: wp.array(dtype=wp.vec3),
    faces: wp.array2d(dtype=wp.int32),
    vertex_offsets: wp.array(dtype=wp.int32),
    face_offsets: wp.array(dtype=wp.int32),
    inside_vertices: wp.array(dtype=wp.int32),
    num_inside: wp.array(dtype=wp.int32),
    bvh_id: wp.uint64
):
    vertex_idx = wp.tid()
    vertex_mesh_id = get_mesh_id(vertex_idx, vertex_offsets)
    
    pos = positions[vertex_idx]
    vel = velocities[vertex_idx]
    
    ray_dir = wp.vec3(0.0, 0.0, -1.0)  # fallback direction
    vel_length = wp.length(vel)
    if vel_length > 1e-6:
        ray_dir = wp.normalize(vel)
    
    query = wp.bvh_query_ray(bvh_id, pos, ray_dir)
    face_idx = wp.int32(0)
    num_hits = wp.int32(0)

    while wp.bvh_query_next(query, face_idx):
        face_mesh_id = get_mesh_id(face_idx, face_offsets)
        if face_mesh_id == vertex_mesh_id:
            continue
        # Get triangle vertices
        v0 = positions[faces[face_idx, 0]]
        v1 = positions[faces[face_idx, 1]]
        v2 = positions[faces[face_idx, 2]]
    ...
    # If inside (odd number of hits), add to compacted array
    if (num_hits % 2) == 1:
        inside_idx = wp.atomic_add(num_inside, 0, 1)
        inside_vertices[inside_idx] = vertex_idx

@wp.kernel
def find_collision_triangles(
    positions: wp.array(dtype=wp.vec3),
    velocities: wp.array(dtype=wp.vec3),
    face_ids: wp.array2d(dtype=wp.int32),
    face_normals: wp.array(dtype=wp.vec3),
    inside_vertices: wp.array(dtype=wp.int32),  # Now using compacted array
    vertex_offsets: wp.array(dtype=wp.int32),
    face_offsets: wp.array(dtype=wp.int32),
    vertex_colliding_triangles: wp.array(dtype=wp.int32),
    vertex_collision_offsets: wp.array(dtype=wp.int32),
    ...
    inside_idx = wp.tid()
    vertex_idx = inside_vertices[inside_idx]  # Get actual vertex index
    vertex_mesh_id = get_mesh_id(vertex_idx, vertex_offsets)
    
    pos = positions[vertex_idx]
    vel = velocities[vertex_idx]
    
    direction = wp.vec3(0.0, 0.0, -1.0)  # fallback
    vel_length = wp.length(vel)
    if vel_length > 1e-6:
        direction = wp.normalize(-vel)
    

    face_id = int(0)
    query = wp.mesh_query_point_no_sign(mesh_id, pos, 1.0)
    if (query.result):
        face_mesh_id = get_mesh_id(query.face, face_offsets)
        if face_mesh_id == vertex_mesh_id:
            vertex_collision_counts[vertex_idx] = 0
            return
            
        normal = face_normals[face_id]
        if wp.dot(normal, direction) > 0.0:
            # Get vertices of nearest triangle to create AABB
            v0 = positions[face_ids[face_id, 0]]
            v1 = positions[face_ids[face_id, 1]]
            v2 = positions[face_ids[face_id, 2]]
            
            # Create AABB around triangle with neighbor_radius
            lower = wp.vec3(
                wp.min(wp.min(v0[0], v1[0]), v2[0]) - neighbor_radius,
                wp.min(wp.min(v0[1], v1[1]), v2[1]) - neighbor_radius,
                wp.min(wp.min(v0[2], v1[2]), v2[2]) - neighbor_radius
            )
            upper = wp.vec3(
                wp.max(wp.max(v0[0], v1[0]), v2[0]) + neighbor_radius,
                wp.max(wp.max(v0[1], v1[1]), v2[1]) + neighbor_radius,
                wp.max(wp.max(v0[2], v1[2]), v2[2]) + neighbor_radius
            )
            
            # Query faces within AABB
            offset = vertex_collision_offsets[vertex_idx]
            max_collisions = vertex_collision_offsets[vertex_idx + 1] - offset
            collision_count = 0
            
            # First store the nearest triangle
            if collision_count < max_collisions:
                vertex_colliding_triangles[offset + collision_count] = face_id
                collision_count += 1
            
            # Find neighboring triangles within AABB
            query_aabb = wp.mesh_query_aabb(mesh_id, lower, upper)
            neighbor_face = int(0)
            while wp.mesh_query_aabb_next(query_aabb, neighbor_face) and collision_count < max_collisions:
                if neighbor_face != face_id:  # Skip the original triangle
                    neighbor_mesh_id = get_mesh_id(neighbor_face, face_offsets)
                    if neighbor_mesh_id == face_mesh_id:  # Only collect from same mesh as nearest triangle
                        normal = face_normals[neighbor_face]
                        if wp.dot(normal, direction) > 0.0:
                            vertex_colliding_triangles[offset + collision_count] = neighbor_face
                            collision_count += 1
            
            vertex_collision_counts[vertex_idx] = collision_count
        else:
            vertex_collision_counts[vertex_idx] = 0